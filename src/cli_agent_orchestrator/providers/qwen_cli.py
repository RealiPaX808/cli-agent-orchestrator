"""Qwen CLI provider implementation (Alibaba)."""
...
RESPONSE_PATTERN = r"❯"  # Pattern for Qwen CLI prompt (similar to Gemini)
IDLE_PROMPT_PATTERN = r"❯"  # Pattern for Qwen CLI prompt
IDLE_PROMPT_PATTERN_LOG = r"❯"
PROCESSING_PATTERN = r"[\.\.\.]"  # Processing indicators


class QwenCliProvider(BaseProvider):
    """Provider for Qwen CLI tool integration (Alibaba)."""
    ...
    def initialize(self) -> bool:
        """Initialize Qwen CLI provider by starting qwen command."""
        # Wait for shell to be ready first
        if not wait_for_shell(tmux_client, self.session_name, self.window_name, timeout=10.0):
            raise TimeoutError("Shell initialization timed out after 10 seconds")

        command = f"qwen {self._agent_profile if self._agent_profile else ''}"
        tmux_client.send_keys(self.session_name, self.window_name, command)

        # Wait for Qwen CLI prompt to be ready
        if not wait_until_status(
            self, TerminalStatus.IDLE, timeout=30.0, polling_interval=1.0
        ):
            raise TimeoutError("Qwen CLI initialization timed out after 30 seconds")

        self._initialized = True
        return True

    def get_status(self, tail_lines: Optional[int] = None) -> TerminalStatus:
        """Get Qwen CLI status by analyzing terminal output."""
    ...
    def get_idle_pattern_for_log(self) -> str:
        """Return Qwen CLI IDLE prompt pattern for log files."""
        return IDLE_PROMPT_PATTERN_LOG

    def extract_last_message_from_script(self, script_output: str) -> str:
        """Extract Qwen's final response message using ❯ indicator."""
        # Strip ANSI codes for pattern matching
        ...
        if not matches:
            raise ValueError("No Qwen CLI response found - no ❯ pattern detected")
        ...
        if not remaining_text:
            raise ValueError("Empty Qwen CLI response - no content found")

        # Clean up trailing whitespace and prompts
        final_answer = re.sub(r"❯.*$", "", remaining_text).strip()
    ...
    def exit_cli(self) -> str:
        """Get the command to exit Qwen CLI."""
        return "/bye"

    def cleanup(self) -> None:
        """Clean up Qwen CLI provider."""
        self._initialized = False
