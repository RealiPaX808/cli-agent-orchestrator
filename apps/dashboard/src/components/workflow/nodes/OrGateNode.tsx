...
export const OrGateNode = memo(({ data, selected }: NodeProps) => {
  ...
  const isSplit = nodeData.type === 'or_split';
  const statusType = getStatusType(nodeData.status);

  return (
    <div style={{ position: 'relative' }}>
      <svg width="120" height="100" style={{ position: 'absolute', top: 0, left: 0 }} aria-label={`OR ${isSplit ? 'Split' : 'Join'} Node`}>
        <title>{isSplit ? 'OR Split Gateway - Execute one or more paths' : 'OR Join Gateway - Wait for active paths'}</title>
        <polygon
        ...
        />
        <circle cx="60" cy="50" r="15" fill="none" stroke="#f9f9f9" strokeWidth="2" />
        <text
          x="60"
          y="75"
          textAnchor="middle"
